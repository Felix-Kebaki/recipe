      ...
      <div className="FilterDivWrapper">
        <ul className="FilterStill">
          ...
          <div
            className={
              filter
                ? "FilterLinksOnlyDiv showFilter"
                : "FilterLinksOnlyDiv unshowFilter"
            }
          >
            {categories &&
              categories.map((cat) => (
                <p
                  key={cat.idCategory}
                  id="EachFilterLink"
                  onClick={() => HandleClickOnCategory(cat.strCategory)}
                >
                  {cat.strCategory}
                </p>
              ))}
              ...
              >
                <div className="EachRecipeImageDiv">
                  <img
                    ...
                    id="MainRecipeImage"
                  />
                  <p onClick={HandleClickOfFav}>
                    <FontAwesomeIcon
                      icon={faHeart}
                      className={
                        favouriteList.findIndex(
                          (ok) => ok.idMeal === each.idMeal,
                        ) !== -1
                          ? "FavIconOnRecipe favPage"
                          : "FavIconOnRecipe"
                      }
                      onClick={() => HandleAddToFav(each)}
                    />
                  </p>
                </div>
                <div className="EachRecipeDetailsDiv">
                  <div className="EachRecipeTopLittleDetails">
                    ...
                    <p id="CategoryRecipe">
                      CATEGORY :
                      <span>
                        {each.strCategory ? each.strCategory : searchCat}
                      </span>
                    </p>
                  </div>
